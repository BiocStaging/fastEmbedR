...
double bilinear_grid_value(const std::vector<double>& grid,
                           ...
                           const double gx,
                           const double gy) {
  return bilinear_grid_value_t<double>(grid, grid_size, gx, gy);
}
...
void compute_gradient_fft_grid(const SparseProbabilities& p,
                               ...
                               const double exaggeration,
                               const int n_threads,
                               FftGridWorkspaceT<double>* workspace,
                               std::vector<double>& grad) {
  if (dims != 2) {
  ...
  FftGridWorkspaceT<double> local_workspace;
  FftGridWorkspaceT<double>& ws = workspace == nullptr ? local_workspace : *workspace;
  ...
  compute_fft_grid_convolution_workspace_t<double>(
    ws.mass,
    ws.mass_x,
    ws.mass_y,
    grid_size,
  ...
  parallel_for(n, n_threads, [&](const int begin, const int end, const int thread_id) {
    ...
    for (int i = begin; i < end; ++i) {
      const double q_value = bilinear_grid_value(
        ws.q_grid,
        grid_size,
        ws.gx[static_cast<std::size_t>(i)],
        ws.gy[static_cast<std::size_t>(i)]
      );
      local_sum_q += q_value;
    }
  ...
  parallel_for(n, n_threads, [&](const int begin, const int end, const int) {
    for (int i = begin; i < end; ++i) {
      const std::size_t base = static_cast<std::size_t>(i) * 2u;
      const double px = ws.gx[static_cast<std::size_t>(i)];
      const double py = ws.gy[static_cast<std::size_t>(i)];
      const double q2_value = bilinear_grid_value(ws.q2_grid, grid_size, px, py);
      const double xq2_value = bilinear_grid_value(ws.xq2_grid, grid_size, px, py);
      const double yq2_value = bilinear_grid_value(ws.yq2_grid, grid_size, px, py);
...
void compute_gradient(const SparseProbabilities& p,
                      ...
                      FftGridWorkspaceT<double>* fft_workspace,
                      std::vector<double>& grad) {
  if (repulsion_mode == "fft_grid") {
    compute_gradient_fft_grid(p, y, n, dims, exaggeration, n_threads, fft_workspace, grad);
  } else {
    compute_gradient_pair_symmetric(p, y, n, dims, exaggeration, n_threads, grad);
  }
}
