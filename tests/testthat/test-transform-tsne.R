test_that(paste(
    "transform_tsne places query rows from supplied reference",
    "neighbors"
), {
    set.seed(401)
...
test_that(paste(
    "transform_tsne CPU transform is deterministic across",
    "query-parallel threads"
), {
    set.seed(406)
    n_ref <- 96L
    n_query <- 32L
    ...
    indices <- matrix(
        sample.int(n_ref, n_query * k, replace = TRUE),
        nrow = n_query
    )
    ...
    expect_equal(
        as.numeric(four_threads), as.numeric(one_thread),
        tolerance = 0
    )
...
test_that("transform_tsne CPU batching preserves fixed-reference results", {
    old_batch <- Sys.getenv(
        "FASTEMBEDR_TSNE_TRANSFORM_BATCH_SIZE",
        unset = NA_character_
    )
    on.exit(
        {
...
test_that("transform_tsne reports GPU transform backends honestly", {
    ...
    expect_equal(
        cfg$optimizer,
        "opentsne_style_fixed_reference_transform_metal"
    )
