test_that(paste(
    "fastEmbedR backend precedence is explicit, option,",
    "environment, CPU"
), {
    ...
    options(fastEmbedR.backend = NULL)
    Sys.unsetenv("FASTEMBEDR_BACKEND")
    expect_identical(fastEmbedR_backend(), "cpu")
    Sys.setenv(FASTEMBEDR_BACKEND = "metal")
    expect_identical(fastEmbedR_backend(), "metal")
    options(fastEmbedR.backend = "cuda")
    expect_identical(fastEmbedR_backend(), "cuda")
...
test_that("principal public functions defer omitted backends", {
    ...
    omitted_backend <- vapply(
        functions,
        function(fn) is.null(formals(fn)$backend),
        logical(1)
    )
    expect_true(all(omitted_backend))
})
