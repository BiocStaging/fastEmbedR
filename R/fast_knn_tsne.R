...
tsne_knn <- function(indices,
    ...
    if (fastembedr_is_gpu_knn(indices) && identical(backend, "cuda")) {
        ...
        if (isTRUE(gpu_info$has_self)) {
            stop(
                "CUDA GPU-resident t-SNE requires non-self KNN. ",
            ...
            if (length(n_neighbors) != 1L || is.na(n_neighbors) ||
                ...
                stop(
                    sprintf(
                        ...
                        "available in the GPU KNN object",
                        "."
                    ),
                    call. = FALSE
                )
    ...
    if (inherits(indices, "fastEmbedR_tsne_prepared")) {
        ...
        if (isTRUE(affinity_support_missing) && !is.null(
            indices$affinity_support
        )) {
...
tsne <- function(data,
    ...
    if ("init" %in% names(dots)) {
        stop(
        "`init` is not an argument of `tsne()`; use `Y_init` or `init_data` ",
    ...
    if ("n_neighbors" %in% names(dots)) {
        stop(
            "`n_neighbors` is not an argument of `tsne()`; use `perplexity` ",
    ...
    if (is_knn_input(data)) {
        ...
        if (is.null(Y_init) && !is.null(init_data)) {
            init_backend <- if (optimizer_backend %in% c(
                "metal",
                "cuda"
            )) {
                optimizer_backend
        ...
        if (is.na(knn_backend) || is.null(knn_backend)) knn_backend <-
            "supplied"
        metrics <- data.frame(
            method = "tsne",
            n = n,
        ...
        out <- list(
            ...
            knn = if (isTRUE(keep_knn)) {
                list(indices = knn_result$indices,
    ...
    knn_time <- system.time({
        if (is.null(nn)) {
            ...
            if (fastembedr_is_gpu_knn(raw_knn)) {
                ...
                if (gpu_info$n != n || gpu_info$k < n_neighbors || isTRUE(
                    gpu_info$has_self
                )) {
                    stop(
                    "GPU-resident KNN output is incompatible with t-SNE input.",
                        call. = FALSE
                    )
                }
                knn_result <- list(
            ...
            if (fastembedr_is_gpu_knn(nn)) {
                ...
                if (incompatible) {
                    stop(
                        sprintf(
                            "%s%s%s",
                            "Supplied GPU-resident KNN output ",
                            "is incompatible with t-SNE input",
                            "."
                        ),
                        call. = FALSE
                    )
                ...
                knn_result <- normalize_supplied_knn(nn, n, n_neighbors,
                    keep_self = keep_knn
                )
                knn_result$nn_backend <- attr(nn, "backend")
                if (is.null(knn_result$nn_backend)) knn_result$nn_backend <-
                    "supplied"
            }
    ...
    initialization_time <- system.time({
        ...
        if (is.null(Y_init)) {
            ...
            if (identical(optimizer_backend, "cuda") && fastembedr_is_gpu_knn(
                embedding_knn_input
            )) {
                cuda_init_candidate <- opentsne_pca_input_matrix(init_source)
                if (nrow(cuda_init_candidate) != n) {
                    stop("`init_data` must have one row per input row.",
                        call. = FALSE)
                }
                ...
                if (!is.null(Y_init)) {
                    init_info$method <- attr(Y_init,
                        "fastEmbedR_init_method") %||% "pca"
                    init_info$backend <-
                    attr(Y_init, "fastEmbedR_init_backend") %||% init_backend
                } else {
                    stop(
                        "t-SNE PCA initialization failed for backend `",
                        init_backend,
                        "`: ",
    ...
    out <- list(
        ...
        knn_with_self = if (isTRUE(
            keep_knn)) knn_result$knn_with_self else NULL,
