auto_tune_embedding_policy <- function(n,
    ...
    if (length(n) != 1L || is.na(n) || n < 2L) {
        stop("`n` must be the number of rows and must be at least two.",
            call. = FALSE
        )
    }
    ...
    policy <- list(
        ...
        refinement_selected_by = if (identical(
            mode,
            "auto"
        )) {
            "best_default"
    ...
    if (auto_umap_should_run_pilot(
        ...
        pilot <- tryCatch(
            ...
            error = function(e) {
                list(
                    status = "failed",
                    reason = paste0("pilot auto-tune failed: ",
                        conditionMessage(e))
                )
            }
        )
        if (identical(pilot$status, "success")) {
            ...
            policy$refinement_selected_by <- if (identical(
                policy$landmark_refinement,
                "none"
            )) {
...
auto_umap_should_run_pilot <- function(x,
    ...
    if (is.null(x) || is.null(dim(x)) || nrow(x) != n) {
        return(FALSE)
    }
...
auto_umap_pilot_tune <- function(x,
    ...
    pilot_backend <- auto_umap_pilot_knn_backend(knn_backend, pilot_n, ncol(
        x_pilot
    ), max_k)
    ...
    labels_factor <- if (is.null(labels_pilot)) {
        NULL
    } else {
        as.factor(
            labels_pilot
    ...
    run_config <- function(k, spectral_n_iter, n_epochs, init_scale, stage) {
        ...
        row <- data.frame(
            ...
            avoid_aggressive_k_reduction = isTRUE(
                dataset_profile$avoid_aggressive_k_reduction
            ),
            cache_hit = FALSE,
            status = "success",
            error = NA_character_,
    ...
    optimizer_budget <- if (length(alternate_k_candidates) > 0L &&
        pilot_max_configs >= 4L) {
    ...
    list(
        ...
        scores = scores,
        reason = paste0(
            "pilot auto-tune sampled ", pilot_n,
            " rows, tuned optimizer settings first at k=", base_k,
            ", selected k=", best$k,
            ...
            if (isTRUE(dataset_profile$avoid_aggressive_k_reduction)) {
                paste0("; avoided aggressive k reduction for ",
                    dataset_profile$reason)
            } else {
                ""
            }
        )
...
auto_umap_rank_alternate_k_candidates <- function(candidates,
    ...
    if (length(candidates) == 0L || max_candidates <= 0L) {
        return(integer(0L))
    }
    if (isTRUE(dataset_profile$avoid_aggressive_k_reduction)) {
...
auto_umap_select_pilot_winner <- function(successful,
    ...
    candidates <- candidates[order(-candidates$pilot_score, candidates$elapsed),
        ,
        drop = FALSE
    ]
    best_alt <- candidates[1L, , drop = FALSE]
    if (!auto_umap_k_change_has_strong_evidence(
        fixed_k_best, best_alt,
        labels_available
    )) {
        return(fixed_k_best)
    }
...
auto_umap_k_change_has_strong_evidence <- function(base,
                                                    candidate,
                                                    labels_available = FALSE) {
    score_gain <- as.numeric(candidate$pilot_score) - as.numeric(
        base$pilot_score
    )
    if (!is.finite(score_gain) || score_gain < 0.02) {
        return(FALSE)
    }
...
auto_umap_metric_not_worse <- function(candidate, base, tolerance) {
    ...
    if (!is.finite(candidate) || !is.finite(base)) {
        return(TRUE)
    }
...
auto_umap_knn_hash <- function(indices,
    ...
    auto_umap_raw_hash(serialize(
        list(
            ...
            distances = signif(as.numeric(distances[rows, cols, drop = FALSE]),
                12L)
        ),
        NULL,
        version = 2L
...
auto_umap_dataset_profile <- function(x,
    ...
    if (!is.null(labels) && length(labels) == n) {
        ...
        if (n_labels >= 2L) {
            ...
            highly_imbalanced <- isTRUE(min_class_fraction < 0.05 ||
                imbalance_ratio >=
                20)
        }
    }

    avoid <- isTRUE(low_dimensional) || isTRUE(tabular_like) || isTRUE(
        highly_imbalanced
    )
    reasons <- c(
        if (isTRUE(low_dimensional)) paste0("low-dimensional p=", p),
        if (!isTRUE(low_dimensional) && isTRUE(tabular_like)) {
            paste0(
                "tabular-like p=", p
            )
        },
...
auto_pilot_sample_indices <- function(n,
    ...
    if (isTRUE(rare_protect)) {
        ...
        quotas <- as.integer(pmax(1L, floor(weights / sum(weights) *
            sample_size)))
    } else {
    ...
    while (sum(quotas) > sample_size) {
        ...
        j <- can_drop[which.max(quotas[can_drop] / pmax(1L, group_sizes[
            can_drop]))]
...
auto_umap_pilot_label_scores <- function(layout,
                                        ...
                                        seed = 4L) {
    if (is.null(labels)) {
        return(list(label_knn_accuracy = NA_real_,
            rare_class_recall = NA_real_))
    }
    labels <- as.factor(labels)
...
auto_umap_pilot_score <- function(structure,
    ...
    label_acc <- if (is.null(
        label_scores
    )) {
        NA_real_
    } else {
        label_scores$label_knn_accuracy
    }
    rare_recall <- if (is.null(
        label_scores
    )) {
        NA_real_
    } else {
        label_scores$rare_class_recall
    }
    ...
    if (isTRUE(has_labels)) {
        ...
        weights <- c(
            trust = 0.25, preserve = 0.20, continuity = 0.15, label = 0.25,
            rare = 0.15
        )
    } else {
        values <- c(
        ...
        weights <- c(
            trust = 0.25, preserve50 = 0.35, continuity50 = 0.25,
            preserve = 0.15
        )
    }
    ...
    if (isTRUE(dataset_profile$avoid_aggressive_k_reduction)) {
        base <- auto_embedding_k(dataset_profile$full_n, "umap",
            include_self = FALSE
        )
    ...
    runtime_penalty <- if (is.finite(elapsed)) {
        0.01 * log1p(as.numeric(
            elapsed
        ))
    } else {
        0
...
auto_umap_pilot_refinement_strength <- function(best) {
    ...
    if (is.finite(trust) && is.finite(preserve) && trust >= 0.96 && preserve >=
        0.80) {
        return(0.5)
    }
...
auto_umap_knn_pilot_tune <- function(indices,
    ...
    if (full_n < pilot_min_n || ncol(indices) < 2L) {
        return(list(
            status = "skipped",
            reason = paste0(
                "KNN pilot auto-tune skipped below threshold or with too few ",
                "neighbors"
            )
        ))
    }
    ...
    if (length(k_candidates) == 0L) {
        return(list(
            status = "skipped",
            reason = paste0(
                "KNN pilot auto-tune skipped because no valid k candidates ",
                "were available"
            )
        ))
    }
    ...
    if (nrow(subgraph$indices) < 50L) {
        return(list(
            status = "skipped",
            reason = paste0(
                "KNN pilot auto-tune skipped because the sampled graph was ",
                "too small"
            )
        ))
    }
    ...
    run_config <- function(k, spectral_n_iter, n_epochs, init_scale, stage) {
        ...
        row <- data.frame(
            ...
            embedding_knn_accuracy = unname(structure[
...
auto_knn_pilot_subgraph <- function(indices,
    ...
    fallback_distance <- if (length(finite_dist) == 0L) {
        1
    } else {
        stats::median(
            finite_dist
    ...
    if (!is.finite(fallback_distance) || fallback_distance <=
        0) {
        fallback_distance <- 1
    }
    ...
    for (local_i in seq_len(n_sub)) {
        ...
        row_dst <- distances[global_i, seq_len(min(ncol(distances), max_k)),
            drop = TRUE
        ]
        valid_row_idx <- is.finite(row_idx) & row_idx >= 1L & row_idx <= nrow(
            indices
        )
        row_idx <- row_idx[valid_row_idx]
        row_dst <- row_dst[valid_row_idx]
        local_nb <- map[row_idx]
        keep <- local_nb > 0L & local_nb != local_i & is.finite(row_dst) &
            row_dst >= 0
        ...
        if (length(local_nb) < max_k) {
            ...
            fill_distance <- if (length(row_dst) > 0L && is.finite(max(
                row_dst))) {
                max(row_dst) * 1.25
            } else {
