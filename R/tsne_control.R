# Parameter validation and optimizer-control helpers for openTSNE workflows.
...
opentsne_neighbor_policy <- function(n,
  ...
  if (!is.null(available)) {
    ...
    if (length(available) != 1L || is.na(available) || available < 1L) {
      stop("The supplied KNN object has no usable non-self neighbor columns.", call. = FALSE)
    }
  }
  if (is.null(perplexity)) {
