...
structure_score_with_backend <- function(layout,
    ...
    if (metal_metric_requested(backend)) {
        if (!metal_metric_available()) {
            reason <- append_metric_backend_reason(reason,
                "metal_scoring_unavailable")
        } else if (ncol(layout) != 2L) {
            reason <- append_metric_backend_reason(
                reason,
                "metal_scoring_requires_2d_layout"
            )
        } else if (preserve_k > 64L) {
            reason <- append_metric_backend_reason(
                reason,
                "metal_scoring_supports_at_most_64_neighbors"
            )
        } else {
            attempt <- capture_error(
...
silhouette_score_with_backend <- function(labels_int,
    ...
    if (metal_metric_requested(backend)) {
        if (!metal_metric_available()) {
            ...
            reason <- append_metric_backend_reason(
                reason,
                "metal_silhouette_requires_2d_layout"
            )
        } else if (n_label_levels > 128L) {
            reason <- append_metric_backend_reason(
                reason,
